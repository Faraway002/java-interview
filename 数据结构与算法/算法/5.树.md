...
做这一题需要清楚地知道什么是左叶子，对于任意一个节点是否为叶子是很容易判断的，就是它没有左右孩子；但是它是否为左叶子就需要额外的信息了，也就是需要它的父节点的信息，所以对于一个节点来说，如果它的左孩子不为 null，并且它的左孩子的左右孩子都为 null，那么它的左孩子就是左叶子。
...
![image-20220728103016508](https://cdn.jsdelivr.net/gh/Faraway002/typora/images/image-20220728103016508.png)

本题注意不是判断 B 是否为 A 的子树，而是子结构，这意味着左子树有一部分相同，右子树也有一部分相同时，也算是子结构。

首先我们遍历整棵树 A，如果遍历到的节点和 B 的根节点相等，说明 B 有可能是 A 的子结构，则进行进一步判断；否则，B 不可能是 A 的子结构，返回 false。

进一步判断：

* 对于以遍历到的节点为根节点的树来说，因为根节点 rootA = B，因此我们继续判断 B 的左右子树和 A 的左右子树。

* 如果 A 的左右子树不空，但是 B 的左右子树都为空，则满足子结构的要求。
* 如果 A 的左右子树空，但是 B 的左右子树不空，则不满足子结构的要求。
* 如果 A 和 B 的左右子树都不空，则重复上述步骤
...
那么，什么是祖先呢？**祖先的定义是：若 p 在节点 root 的子树中，或者 p == root，则 root 是 p 的祖先**。

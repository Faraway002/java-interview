...
4. 明确 dp 数组的含义：`dp[i][j][k]`，表示第 i 天，交易次数限制为 j，股票持有状态为 k 状态下的最大利润，此时能获得的最大利润就是 `dp[n - 1][k][0]`，其中 n 表示共有多少天。

   为什么不是 `dp[n - 1][k][1]`？这是因为这表示你还持有股票，肯定是把它卖掉利润更高一些，也就是最后没有持股的状态才能达到最大利润。

状态转移方程：

* `dp[i][j][0] = Math.max(dp[i - 1][j][0], dp[i - 1][j][1] + prices[i])`，因为 0 表示未持股，所以有两种情况：前一天也未持股或者持股但是卖出了。
* `dp[i][j][1] = Math.max(dp[i - 1][j][1], dp[i - 1][j - 1][0] - prices[i])`，因为 1 表示持股，有两种情况：前一天也持股或者前一天未持股但是买入了。
    ...
    for (int i = 0; i < n; i++) {
        for (int j = k; j >= 1; j--) {
            ...
            if (i == 0) {
                // 第 0 天未持股，利润为 0
                dp[i][j][0] = 0;
                // 第 0 天持股，等价于买下，于是利润为 -price[i]
                dp[i][j][1] = -prices[i];
                continue;
            }
            // 状态转移方程
            // 0 表示未持股，有两种情况：前一天也未持股 或者 持股但是卖出了
            dp[i][j][0] = Math.max(dp[i - 1][j][0], dp[i - 1][j][1] + prices[i]);
            // 1 表示持股，有两种情况：前一天也持股 或者 前一天未持股但是买入了
...
这题相当于[力扣 188. 买卖股票的最佳时机 IV](#力扣 188. 买卖股票的最佳时机 IV) 的限制交易次数为 1 次（k = 1）的版本，那么状态转移方程就变为：

```java
// 0 表示未持股，有两种情况：前一天也未持股 或者 持股但是卖出了
dp[i][0] = Math.max(dp[i - 1][0], dp[i - 1][1] + prices[i]);
// 1 表示持股，有两种情况：前一天也持股 或者 前一天未持股但是买入了，由于只能一次买入，因此直接为 -price
...
首先，我们来看看递归是如何解决这个问题的，对于一个序列 1~n，要计算它能组成多少棵二叉搜索树。

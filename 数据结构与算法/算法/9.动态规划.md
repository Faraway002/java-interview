...
![image-20220713145126431](https://cdn.jsdelivr.net/gh/Faraway002/typora/images/image-20220713145126431.png)
...
本题最基本的就是要知道怎么样判断有效括号，这其实我们之前也讲过了，利用栈就好了。
...
本题乍一看好像是和二叉搜索树的递归类型的题，其实是一个动态规划的题目。
...
![image-20220712120105435](https://cdn.jsdelivr.net/gh/Faraway002/typora/images/image-20220712120105435.png)
...
![image-20220709152721521](https://cdn.jsdelivr.net/gh/Faraway002/typora/images/image-20220709152721521.png)

本题最直观的想法当然是暴力法，把投一个骰子看做是一个独立事件，那么可以用乘法法则计算每一种可能出现的概率。

n 个筛子的点数范围为：[n, 6n]，所以有 5n + 1 个情况要计算，每一个情况可能有多种子情况，复杂度很高。

现在，假设我们已经知道 $n - 1$ 个骰子的情况了，如何求 $n$ 个骰子投出点数 $x$ 的情况呢？

假设新加的这一个骰子投出了 1，那么前 $n - 1$ 个骰子的点数和就应该是 $x - 1$，以此类推，最后把六种情况加起来即可。

即：
$$
p = \sum_{i = 1}^{6}\frac{1}{6}f(n, x - i) = \frac{1}{6}\sum_{i = 1}^{6}f(n - 1, x - i)
$$
其中，$f(n, x)$ 表示 n 个骰子投出点数和为 x 的概率，之所以乘以 $\frac{1}{6}$ 是因为当前新加的骰子投出任意一个值的概率都是 $\frac{1}{6}$。

那么本题就可以使用动态规划求解了，状态转移方程就是上面的公式。
...
![image-20220718174819450](https://cdn.jsdelivr.net/gh/Faraway002/typora/images/image-20220718174819450.png)

本题最暴力的方式自然是穷举，但是穷举是很复杂的，因为你戳破一个气球之后，对于其他气球来说相邻的气球发生变化了，很类似于约瑟夫环问题。

但是本题又会存在很多子问题，因为在不同情况下戳破同一个气球，旁边两个气球相对于其他情况没有改变的情况也是存在的，这就意味着有大量重复子问题，因此我们考虑动态规划。

我们能够运用动态规划算法的一个重要条件是：**子问题必须独立**。而当前题目中我们每戳破一个气球 nums[i]，得到的分数和该气球相邻的气球 nums[i-1] 和 nums[i+1] 是有相关性的。

也就是说如果想要用动态规划，必须巧妙地定义 dp 数组的含义，避免子问题产生相关性，才能推出合理的状态转移方程。

题目说可以认为 nums[-1] = nums[n] = 1，那么我们先直接把这两个边界加进去，形成一个新的数组 ballons：
...
问题就变成了戳破 ballons(0, ballons.length - 1) 开区间内的所有气球，能够得到的最大分数。

我们就把 dp 数组设置为上面的含义，这样一来，获得分数就之和区间开头和结尾有关了，题目要求的结果就是 `dp[0][n + 1]` 的值。

由于我们是开区间，因此对于区间 (i, j) 内任意一个点 k，如果我们知道 `dp[i][k]` 和 `dp[k][j]`（一定要注意是开区间）的值，就意味着这个区间内只剩下 i，j，k 三个气球，求出最后戳爆 k 得到的分数也就很简单了，而由于 `dp[i][k]` 和 `dp[k][j]` 都是最优的，因此我们得到的结果也是最优的。

![image-20220718183803513](https://cdn.jsdelivr.net/gh/Faraway002/typora/images/image-20220718183803513.png)

但是这个 k 可以任选，因此我们要遍历所有 k，得到一个全局最优解。

状态转移方程：`dp[i][j] = dp[i][k] + dp[k][j] + ballons[i] * ballons[j] * ballons[k]`。

因为对于 `dp[i][j]` 来说，我们希望先知道 `dp[i][k]` 和 `dp[k][j]` 的值，而 i < k < j，因此 这是一个从后往前遍历的 dp，i 应该从 n 开始，j 则应该从左往右：

![image-20220718184358627](https://cdn.jsdelivr.net/gh/Faraway002/typora/images/image-20220718184358627.png)
...
public int maxCoins(int[] nums) {
    ...
    for (int i = n; i >= 0; --i) {
        for (int j = i + 1; j <= n + 1; ++j) {
          	// 注意是开区间
            for (int k = i + 1; k < j; ++k) {
                dp[i][j] = Math.max(dp[i][k] + dp[k][j] + ballons[i] * ballons[j] * ballons[k], dp[i][j]);
            }
        }
    }

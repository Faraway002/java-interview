...
从本质上来说，**注解是一种标签，其实质上可以视为一种特殊的注释，如果没有解析它的代码，它并没有任何的作用。**
...
**其本质是一个继承了 `Annotation` 接口的接口，但是不能继承其他接口或者注解**。`Annotation` 本身并不是注解。
...
更特殊的是，如果注解有一个方法名为 `value`，那么可以直接使用 `@AnnotationName(value)` 的方式把值 `value` 传递给方法 `value`，而不需要使用方法名，这叫做**单值注解**。单值注解只适用于**只需要给 `value` 方法赋值的时候**。
...
>
> ![image-20220204163732635](https://cdn.jsdelivr.net/gh/Faraway002/typora/images/image-20220204163732635.png)
>

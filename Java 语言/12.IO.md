...
以 Web 服务端处理网络请求为例，看一下处理的流程：

![image-20220308201403327](https://cdn.jsdelivr.net/gh/Faraway002/typora/images/image-20220308201403327.png)

现代应用程序主要是进行网络 I/O 以及磁盘 I/O，两者的速度比内存的速度要慢上 10 ~ 10w 倍，更不用与 CPU 的速度对比了，因此，如何优化 I/O 是现代应用程序的一大难题。

## Java 的三种 I/O 模型

Java 共有三种 I/O 模型：

![image-20220309153234659](https://cdn.jsdelivr.net/gh/Faraway002/typora/images/image-20220309153234659.png)

### BIO（Blocking I/O）

阻塞 I/O，`java.io` 包中的类提供了这些功能，交互方式是同步、阻塞的。也就是说，在读取输入流或者写入输出流时，在读、写动作完成之前，线程会一直阻塞在那里，它们之间的调用是可靠的**线性顺序**。

很多时候，人们也把 `java.net` 下面提供的部分网络 API，比如 `Socket`、`ServerSocket`、`HttpURLConnection` 也归类到同步阻塞 I/O 类库，因为网络通信同样是 I/O 行为。

* 优点是代码比较简单、直观。

* 缺点则是 I/O 效率和扩展性存在局限性，容易成为应用性能的瓶颈。
...
如果要让服务端能够处理多个请求，则必须使用多线程，在高并发的场景下，多个线程的开销极大，即使可以采用线程池优化性能，但是开销仍然很大，严重时甚至会发生 OOM。
...
事实上，`java.nio` 包不仅仅提供了非阻塞 I/O 实现，还提供了 I/O 多路复用的支持。
...
文件和目录路径的抽象表示，如果希望在程序中操作文件和目录，则可以通过该类来完成。
...
* 相对路径：指的是以当前目录为根目录的文件路径，比如当前目录是 `C:\Users\94585`，则 `123` 表示 `C:\Users\94585\123` 这个目录。
  ...
  * `..`：表示当前目录的上一级目录，上面的例子中 `123` 也可以写成 `..\\Users\\123`。
...
* `RandomAccessFile(String name, String mode)`：使用 mode 打开 name 文件。
* `RandomAccessFile(File file, String mode)`：使用 mode 打开文件。
...
rw 是会使用缓存的，只有缓存满了才会写入到磁盘中，这对于调试时很不友好，文件内容可能并不会立刻反映到文件中；rws 则是立即更新到磁盘中，性能就会稍微差一点；而 rwd 是一个折中的选择，会理解写入文件内容，但是不会即时更新文件描述符。
...
- `System.err`：一个 `PrintStream` 流。System.err 与 System.out 的运行方式类似，但它更多的是用于打印错误文本。一些 IDE 为了让错误信息更加显眼，会将错误信息以红色文本的形式通过 System.err 输出到控制台上。
- `System.in`：一个典型的连接控制台程序和键盘输入的 `InputStream` 流。通常用于当数据是通过命令行参数或者配置文件传递给命令行 Java 程序的时候。
...
流从概念上来说是一个**连续的数据流**。可以把流想象为一个水管，源头和目的地是两个水桶，水就是数据；在两个水桶中，水通过水管流动，每个水滴就是一个字符或一个字节。在流中，有一个指针指向当前操作位置，指针可以向前或向后移动，表示数据的流向。
...
这两个类都实现了 `Closeable` 接口，而该接口又继承自 `AutoCloseable` 接口，这些接口表示可以自动关闭资源，且只定义了一个 `close` 方法用于关闭资源。
...
缓冲流**通过内置一个缓冲数组，通过读写缓冲区，来减少操作系统的 I/O 次数。**

一般来说，都会使用缓冲流来包装其他的流，以增强性能。在构造缓冲流时，可以指定缓冲区的大小。
...
在集合框架中，我们看到底层存储数据的成员都加上了这个关键字，目的就是为了防止默认序列化机制序列化一个集合而导致开销过大。而集合框架中的类都实现了自定义序列化机制，我们很快就会了解到。
...
`Socket` 和 `ServerSocket` 提供了对 TCP 的支持，其中 `Socket` 一般用于 TCP 客户端，而 `ServerSocket` 用于 TCP 服务端。
...
> **BIO 和 NIO**
>
> BIO 在 NIO 推出以后，已经使用 NIO 重新实现过了，大多数情况下 BIO 的性能还要更好。但是这仅限于文件 I/O。
>
...
`File` 与 `Socket` 通常被认为是比较高层次的抽象，而 `Channel` 则是更加操作系统底层的一种抽象，这也使得 NIO 得以充分利用现代操作系统底层机制，获得特定场景的性能优化，例如，DMA（Direct Memory Access）等。不同层次的抽象是相互关联的，比如我们可以通过 `Socket` 获取 `Channel`，反之亦然。
...
![image-20220312160436917](https://cdn.jsdelivr.net/gh/Faraway002/typora/images/image-20220312160436917.png)

如果不重置 position 为 0，往后读自然读不到任何数据。

翻转以后，就可以从头开始读了，此时 limit 由 1024 变成了 13：

![image-20220312160622429](https://cdn.jsdelivr.net/gh/Faraway002/typora/images/image-20220312160622429.png)
...
![image-20220312170926826](https://cdn.jsdelivr.net/gh/Faraway002/typora/images/image-20220312170926826.png)
...
要判断一个 `Channel` 能被 `Selector` 复用，则可以看它**是否继承了 `SelectableChannel`**。
...
![image-20220313231337885](https://cdn.jsdelivr.net/gh/Faraway002/typora/images/image-20220313231337885.png)
...
![image-20220313231229544](https://cdn.jsdelivr.net/gh/Faraway002/typora/images/image-20220313231229544.png)

NIO 模式：

![image-20220313231250871](https://cdn.jsdelivr.net/gh/Faraway002/typora/images/image-20220313231250871.png)	